...
import { MapPin, Bed, Bath, Square, FileText, CheckCircle2, Clock } from "lucide-react";
...
export default function ClientDashboardPage() {
  ...
  if (!order || !order.property) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center max-w-lg mx-auto">
        <h2 className="text-3xl font-bold mb-4">No Property Selected</h2>
        <p className="text-muted-foreground mb-8">
          It looks like you haven't selected a property yet. Please browse our listings and choose one to get started.
        </p>
        <Link href="/onboarding/select-property" className={cn(buttonVariants({ variant: "default" }), "bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-8")}>Browse Properties</Link>
      </div>
    );
  }
  ...
  const agentRole = order.agent_role || (order.agent && order.agent.role) || "Property Specialist";
    ...
    <div className="flex flex-col gap-8 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Here is the status of your selected property.</p>
        </div>
        <Link href="/client-dashboard/orders" className={cn(buttonVariants({ variant: "outline" }), "glass font-medium")}>
          View All Orders
        </Link>
      </div>
          ...
          <Card className="glass-card border-none shadow-xl overflow-hidden group">
            ...
            <CardContent className="p-8">
              <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-4">
                <div>
                  <h2 className="font-extrabold text-3xl mb-2">{property.title}</h2>
                  <div className="flex items-center text-muted-foreground text-base">
                    <MapPin className="h-4 w-4 mr-2 shrink-0" />
                    <span>{property.address}</span>
                  </div>
                </div>
                <span className="font-extrabold text-primary text-3xl shrink-0">{property.price}</span>
              </div>
              ...
              <div className="flex items-center gap-6 text-sm pt-6 mt-6 border-t border-white/10">
                <div className="flex items-center gap-2 font-medium">
                  ...
                  <div className="flex flex-col"><span className="text-xl font-bold">{property.beds}</span><span className="text-muted-foreground text-xs uppercase tracking-wider">Beds</span></div>
                </div>
                <div className="flex items-center gap-2 font-medium">
                  <div className="p-2 rounded-lg bg-primary/10 text-primary"><Bath className="h-5 w-5" /></div>
                  <div className="flex flex-col"><span className="text-xl font-bold">{property.baths}</span><span className="text-muted-foreground text-xs uppercase tracking-wider">Baths</span></div>
                </div>
                <div className="flex items-center gap-2 font-medium">
                  <div className="p-2 rounded-lg bg-primary/10 text-primary"><Square className="h-5 w-5" /></div>
                  <div className="flex flex-col"><span className="text-xl font-bold">{property.sqft}</span><span className="text-muted-foreground text-xs uppercase tracking-wider">Sqft</span></div>
                </div>
              </div>
            </CardContent>
          </Card>
